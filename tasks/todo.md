...
- [ ] **#4 · Deal Scoring** (`/tools/deal-scoring`, hoy PlaceholderPage) — **IP recuperado** (`docs/Modelos Propietarios/DealScoringAgent_Development_Spec.docx` + `deal_scoring.py`): XGBoost de prob. de cierre con explicabilidad + narrativa Claude. **Bloqueo de datos verificado:** no hay fuente pública que entrene el modelo (4/10 features = scores propietarios de analista; sesgo de supervivencia → faltan negativos; ToS Crunchbase prohíbe entrenar). **Estrategia decidida (2026-06-20):** capturar el dato propio → ≥200 deals etiquetados. Scaffold del registro `HistoricalDeal` **iniciado y verificado** (G3 abajo).
  - [x] **G3·a — scaffold del registro** ✅ (pendiente de merge): `modules/deal_scoring/` (modelo `HistoricalDeal` = 10 features + outcome + trazabilidad), migración `f8b3d1a6c904` (verificada upgrade+rollback en SQLite), seed **anonimizado** de 35 oportunidades (3 cerrados / 2 negativos / 30 abiertos), tests 5/5, script de backfill guiado. **Seed con nombres reales NO se versiona** (repo público → `docs/Modelos Propietarios/` + `docs/Casos Entrenamiento/` gitignoreados; mapping privado en `SDQ_Deal_Register_Inventario.xlsx`).
  - [ ] **G3·b — correr la migración `f8b3d1a6c904` en prod** (Railway/Postgres) y cargar el seed.
  - [ ] **G3·c — backfill de las 4 señales de analista** (promoter/financial/market/regulatory, juicio SDQ vía `scripts/backfill_deal_scores.py`) + confirmar outcomes → poblar hacia **N≥~50** primero, luego hacia 200.
  - [ ] **G3·d — `api/` + `router/` + evaluator XGBoost** conectado a `HistoricalDeal` filtrando `closed_successfully IS NOT NULL`; `model_version` traza la etapa (reducido 6-features cold-start → full 10 cuando N≥200, balance 40–60%). Hasta entonces, cualquier score es heurístico y se marca como tal (doctrina anti-fabricación).
