...
- [ ] **#4 · Deal Scoring** (`/tools/deal-scoring`, hoy PlaceholderPage) — **EN CURSO.** IP recuperado (spec XGBoost prob. de cierre + explicabilidad + narrativa Claude). **Análisis profundo de 2 backups (OneDrive 37 + LaCie 32 carpetas, ~5,300 docs + email, vía workflows multi-agente, 2026-06-21):** ~176 oportunidades → **~29 labels validados con evidencia, balanceados (~19 closed / ~10 lost)** en 2 poblaciones (A: deals SDQ advisory/capital-raise; B: real-estate dev de CBS). **Decisión del dueño: modelar A+B con `deal_type` como feature.** 100 abiertos = pipeline de recuperación. Criterio de graduación: curva de aprendizaje + IC de AUC en CV (NO un "200" arbitrario — sin sustento; el propio proto muestra AUC inestable a N=200). Ver [[deal-scoring-ip-and-data]].
  - [x] **G3·a — scaffold del registro** ✅: `modules/deal_scoring/` (modelo `HistoricalDeal`), migración `f8b3d1a6c904`, seed anonimizado, backfill, tests. Seed real NO se versiona (gitignoreado).
  - [ ] **G3·b — migración en prod + cargar el set maestro validado** al registro (`HistoricalDeal`).
  - [ ] **G3·c — rúbrica anclada a los 7 ejes** (scorea ya, badge "rúbrica declarada") + backfill de las 4 señales de analista (juicio SDQ).
  - [ ] **G3·d — página + evaluator** (rúbrica ahora; XGBoost cuando la curva de aprendizaje gane el umbral de IC en CV).
