...
> **Cuarta actualización.** El panel pasó de **1 caso comparable a 6**. Faltan **dos**. Este
> documento ya no es un pedido abierto: son dos huecos concretos con su fuente nombrada.
...
**Meta: 8 casos sobre base contable. Hoy hay 6.** Faltan dos.

## Por qué importa

Alimenta el eje de valuación de entidades, que valúa contra **patrimonio contable**. Sin ocho
casos comparables, el eje publica su modelo y declara que **no está contrastado contra precios
pagados**, y la vista de fusiones y adquisiciones queda cerrada.
...
### Los SEIS casos COMPARABLES (base contable)
...
El rango va de **0,77× a 2,53×**, y que haya un caso por debajo de 1,0× importa: un panel
compuesto solo de primas estaría sesgado por selección.

### Las TRES fuentes de denominador que funcionaron
...
3. **Los estados auditados de la propia adquirida**, cuando cotizaba. Cerró Cayman National:
   el denominador no depende de lo que el comprador decida contar.
...
## Los DOS que faltan: vías abiertas con lo que le falta a cada una

### Grupo A — el precio ya está verificado, falta el denominador

Es donde queda el mayor rendimiento: la mitad cara del trabajo está hecha.

| operación | precio verificado | qué falta | dónde buscarlo |
|---|---|---|---|
...
| **CIHL / Scotiabank Belize** (cierre abr-2021) | **US$20 M** (final) | patrimonio contable | memoria 2021 de CIHL, o Banco Central de Belice |
| **NCB / Clarien** (Bermuda, 2017) | J$4,15 mm ≈ US$33,04 M por el 50,1 % | ver abajo | Bermuda Monetary Authority |

- **IBL** es compra **por etapas**: el precio es solo del segundo tramo (50 %→100 %).
- **Butterfield Barbados**: se conocen activos (US$308 M) y depósitos (US$270 M), que **no son
  patrimonio**. De la ganancia neta de US$7,2 M sobre US$45 M brutos se despejaría un libro de
  ~US$37,8 M, pero eso es el *carrying value consolidado del vendedor* —con su goodwill
  asignado y sus diferencias de cambio—, no el patrimonio de la entidad. Es derivación, no
  dato.
- **Belize**: ojo, el precio del anuncio (hasta US$30,5 M) **no** fue el del cierre (US$20 M).
- **Clarien**: falta primero saber **qué fue la operación**. NCBFG *suscribió* el 50,1 % —un
  aporte de capital que entra a la sociedad— y no está establecido que le comprara acciones a
  un accionista. Si fue suscripción, el monto no es un precio de control.

### Grupo B — la fuente existe y no se abrió

- **Sagicor Group Jamaica / RBC Royal Bank (Jamaica) (2014).** Las tres cifras aparecen y
  **cierran entre sí** —consideración US$84,378 M, activos netos US$113,429 M, goodwill
  negativo US$29,051 M— y reconcilian con los J$9.500 millones de la prensa. Pero vienen de un
  resumen, no del estado auditado. **Hay que abrir la nota.** Y aun abierta, «net assets
  acquired» es valor razonable: solo contaría para la meta si la nota publica además el libro.
- **Santander BanCorp — base contable.** El caso ya está en el panel sobre valor razonable.
  Para pasarlo a contable hace falta el patrimonio consolidado de la **tenedora**, que es lo
  que compró el precio. El Call Report del FDIC solo cubre el banco (US$1.013,608 M a
  jun-2020) y usarlo cruzaría alcances — daría 1,26× y parecería un P/B. La vía es el **FR
  Y-9C** ante la Reserva Federal.

### Grupo C — barrido que todavía no se hizo

**Resoluciones de la Junta Monetaria dominicana, año por año, 2016→2026.** Es el camino más
corto: para una operación dominicana el denominador ya lo tenemos. De cada resolución que
autorice venta o traspaso de acciones hace falta **la entidad, la fecha de cierre, el
porcentaje y el monto**. Nada más.
