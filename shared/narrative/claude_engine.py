...
THIN_TEMPLATES = {
    ...
    "brand_plan_readiness": (
        ...
        "Mercado: la lectura del plan que el cliente compartió, cruzada con lo que el "
        "instrumento de medición puede certificar hoy.\nContexto:\n{context}\n\n"
        "Máximo 350 palabras, prosa corrida, SIN encabezados (el documento ya titula la "
        "sección). Cubre, en este orden de prioridad: (1) el INSIGHT del plan — qué "
        "persigue y en qué se juega, leyendo 'metas_extraidas' y 'compromisos'; conecta "
        "con la tensión del trimestre SIN repetir lo que las otras secciones ya "
        "narraron; (2) qué compromisos puede CERTIFICAR el instrumento hoy — usa el "
        "veredicto mecánico servido en cada fila de 'compromisos' (feasible/gap/"
        "umbral detectable); aquí SÍ clasifica y prioriza con juicio propio: cuál brecha "
        "importa más y por qué; (3) qué falta para medir el resto y A QUIÉN pedírselo: "
        "cortes del tracker → al proveedor en la próxima entrega; series/olas → a la "
        "carga de SDQ; metas de fuentes externas → al cliente o sus plataformas. "
        ...
        "estado evaluable/inevaluable y el mínimo detectable de cada compromiso ya "
        "vienen calculados en el contexto; nárralos y priorízalos, NUNCA los recalcules "
        "ni los contradigas — la aritmética muestral no es tuya. REGLA DURA DE CIFRAS: "
        "usa SOLO números servidos en el contexto (metas del plan, umbrales, bases); "
        "una meta que no esté en 'metas_extraidas' ni en 'compromisos' NO existe — no "
        "narres intenciones del cliente que nadie registró."
    ),
    "deal_outlook": (
