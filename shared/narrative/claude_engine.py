...
THIN_TEMPLATES = {
    ...
    "banking_forensic": (
        "Escribe una LECTURA FORENSE RETROSPECTIVA de una entidad financiera que SALIÓ del "
        "sistema (una quiebra ya consumada) — 'la anatomía de la quiebra'. Es análisis "
        "histórico sobre dato mensual real de la SB (Cronología SB), NO una calificación "
        "emitida en su momento.\nContexto:\n{context}\n\n"
        "Estructura EXACTAMENTE con estos encabezados markdown:\n"
        "## Lectura central — el veredicto en 3-4 frases: cuándo empezó el deterioro "
        "detectable ('onset') y cuántos meses antes del colapso.\n"
        "## Anatomía del deterioro — la secuencia que cuentan los ratios: el salto de "
        "morosidad, el agotamiento de la cobertura de provisiones, la fuga de depósitos; "
        "nombra las cifras y sus fechas del contexto.\n"
        "## Qué habría marcado el modelo — la lectura del backtest de Alerta Temprana: qué "
        "cluster de señales convergió y cuándo.\n"
        "## Lección — el 'y por tanto' metodológico: si el cluster llegó tarde o no llegó, la "
        "contabilidad ocultó el deterioro (los ratios detectan insolvencia, no fraude).\n\n"
        "REGLA DURA DE CIFRAS: usá SOLO los números del contexto (onset, lead_months, "
        "morosidad, cobertura, fuga de depósitos, fechas); NO inventes ni derives cifras que no "
        "estén dadas; si una relación no está, exprésala en palabras SIN número. LÍMITES: el "
        ...
        "como hecho probado salvo que el contexto lo marque; el punto es que los ratios NO lo "
        "ven. Máximo ~600 palabras, denso, sin relleno."
    ),
    "entity_rating": (
