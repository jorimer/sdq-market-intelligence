...
THIN_TEMPLATES = {
    ...
    "anuario_sistema": (
        ...
        "'por_tipo' trae el cambio MEDIANO de cada tipo de entidad, ya computado y ordenado. "
        "Es lo que más informa del año y conviene desarrollarlo primero. Nombrá cada estrato "
        "por su 'tipo_label', nunca por la clave técnica. 'extremos' son las COLAS: "
        ...
        "TÍTULOS DE SECCIÓN DESCRIPTIVOS Y SOBRIOS: nombran el asunto, no lo dramatizan "
        "('Cambio del año por tipo de entidad', nunca 'La fractura estructural'). Evitá los "
        "giros de comentarista y los anglicismos ('intraanual', no 'intrayear').\n\n"
        "No recalcules ninguna cifra: todas vienen resueltas. Si una relación no está servida, "
        "expresala en palabras sin número."
    ),
    "feed_delta": (
        "Escribí el MOVIMIENTO DEL PERÍODO de este eje a partir de su feed sub-anual.\n"
        "Contexto:\n{context}\n\n"
        "Máximo 220 palabras por emisor. 'lecturas_por_emisor' trae UNA entrada por emisor, "
        "cada una con SU período leído ('periodo_leido'), sus series y sus desgloses. Tratá a "
        "cada emisor por separado y nombralo: no los promedies, no los sumes y no elijas uno. "
        "El período del índice anual de este informe "
        "('periodo_del_indice_anual_del_informe') es OTRO: no los confundas ni los sumes.\n\n"
        "EL PERÍODO LEÍDO SE NOMBRA: usá 'periodo_leido' tal cual. Puede no ser el mes en "
        "curso, así que no escribas 'este mes', 'el mes pasado' ni 'la lectura más reciente'. "
        "La sección ya abre con una línea que nombra el período y, si 'fuente_al_dia' es "
        "falso, declara cuándo publicó la fuente por última vez: NO la repitas ni la "
        "contradigas.\n\n"
        "LA REGLA DE ALCANCE ES DURA: leé el movimiento y su magnitud. **NO expliques su "
        "causa.** No hay citación por oración en este producto —la atribución es por sección "
        "y fuente—, así que 'la caída responde a X' es exactamente lo que no se puede "
        "respaldar. No escribas 'debido a', 'responde a', 'por efecto de', 'explicado por' ni "
        "ninguna otra atribución de causa. Si te sale escribir un porqué, no lo escribas.\n\n"
        "TODAS LAS RELACIONES VIENEN RESUELTAS. 'movimiento.direccion' dice si sube o baja y "
        "'movimiento' trae la variación con su MEDIDA ya elegida: copiala. Una serie cuya "
        "medida es 'puntos' NO se expresa en porcentaje. Una con 'variacion_pct' en nulo no "
        "tiene variación porcentual y decir una sería inventarla.\n\n"
        "LA LÍNEA BASE SE NOMBRA SIEMPRE. Cada serie trae 'linea_base' con su TIPO y su "
        "período: un flujo contra el MISMO período del año anterior; un stock contra el "
        "último nivel publicado. Escribí contra qué se mide; un porcentaje sin su base "
        "obliga al lector a suponerla y va a suponer mal.\n\n"
        "'ventana_movil_12m', cuando viene disponible, es la magnitud comparable con el "
        "índice anual del informe — es el puente entre este período y el índice, y vale la "
        "pena nombrarlo. Si trae 'disponible' en falso, NO la menciones como cifra: su motivo "
        "dice por qué no existe.\n\n"
        "'series_sin_lectura' se DECLARA en una frase si no está vacío. Callar lo que no se "
        "pudo medir hace pasar una lectura parcial por una completa.\n\n"
        "Los desgloses por dimensión (las claves que terminan en '_del_mes') son del PERÍODO, "
        "no del año: nombralos así, con el sujeto que la clave declara.\n\n"
        "'nota_del_emisor' dice QUÉ mide cada serie y qué no —un indicador líder no es "
        "producción ejecutada; un stock de personas cubiertas no es un flujo—. Respetala.\n\n"
        "TÍTULOS DESCRIPTIVOS Y SOBRIOS: nombran el asunto, no lo dramatizan.\n\n"
        "No recalcules ninguna cifra: todas vienen resueltas. Si una relación no está "
    ...
    "revision_anual": (
        ...
        "LA UNIDAD DE COMPARACIÓN SON LOS AÑOS, no los trimestres. 'serie_de_cierres' trae el "
        "cierre de cada año disponible y 'variaciones' el movimiento de año contra año, con su "
        "dirección y sus cambios de banda ya resueltos. El año POR DENTRO —la serie de sus "
        "trimestres— es OTRO producto: no lo describas acá ni cites trimestres.\n\n"
        "LA TENDENCIA ES EL APORTE DE ESTE INFORME. 'tendencia' trae cuántos años consecutivos "
        "lleva moviéndose en la misma dirección y el cambio neto del horizonte. Un mal año y el "
        "cuarto año consecutivo bajando son decisiones de exposición DISTINTAS, y sin la serie "
        "no se distinguen: decí cuál de las dos es.\n\n"
        "DECLARÁ EL HORIZONTE. 'tendencia.horizonte_disponible' dice cuántos cierres cubre la "
        "serie, y 'por_que_este_horizonte' por qué son ésos. No hables de «siempre» ni de «la "
        "historia de la entidad»: son los años que hay.\n\n"
        "TÍTULOS DE SECCIÓN DESCRIPTIVOS Y SOBRIOS, si usás títulos: nombran el asunto, no lo "
        "dramatizan ('Capital: deterioro sostenido', nunca 'La fractura estructural'). Y no "
        "uses anglicismos: es 'dentro del año' o 'intraanual', no 'intrayear'.\n\n"
        "'balance' trae cada indicador al cierre CONTRA su valor al cierre del año ANTERIOR. "
        "Son STOCKS: el nivel de diciembre no dice nada sin el del diciembre pasado. Elegí los "
        "pocos que MOVIERON la calificación en vez de recorrerlos todos.\n\n"
        "CADA FILA DEL BALANCE TRAE SU VEREDICTO YA RESUELTO ('veredicto': favorable / "
        "desfavorable / estable / no_aplica, con su 'veredicto_por_que'). COPIALO. NO deduzcas "
        "si subir fue bueno: morosidad que sube y solvencia que baja son las DOS deterioros, y "
        "deducirlo es exactamente lo que sale mal. 'no_aplica' significa óptimo intermedio — "
        "ahí no digas ni mejoró ni empeoró. Y usá la 'unidad' de la fila al citar el cambio.\n\n"
        "Un año que falte en la serie se NOTA y se declara: la tendencia es de los cierres que "
        "hay, no de un continuo.\n\n"
        "No recalcules ninguna cifra: todas vienen resueltas. Si una relación no está servida, "
        "expresala en palabras sin número."
    ),
    ...
    "boletin_sistema_pais": (
        ...
        "**NO escribas encabezado ni título**: el país, su corte, su fuente y su norma los "
        "pone el documento antes de tu texto, computados del dato. Empezá directamente "
        "por el análisis.\n\n"
        "**Fechá cada cifra con el corte de SU serie.** Dentro de un mismo país las "
        "métricas se publican con rezagos distintos —la adecuación de capital puede ir un "
        "mes atrás del resto— y escribir «al cierre de julio» sobre una cifra de junio es "
        "dar por real una fecha falsa.\n\n"
    ...
    "anio_del_sistema": (
        ...
        "'por_tipo' trae el cambio MEDIANO de cada tipo de entidad, ya computado. Es lo que "
        "más informa del año y conviene desarrollarlo primero. Nombrá cada estrato por su "
        "'tipo_label', NUNCA por la clave técnica ('aap', 'banca_multiple'). "
        "'conteo_direccion' dice cuántas mejoraron, empeoraron y quedaron estables — son "
        "cifras del sistema, no casos.\n\n"
        "DECLARÁ EL UNIVERSO al menos una vez: cuántas entran al orden y cuántas quedan fuera "
        "por año incompleto.\n\n"
        "TÍTULOS DE SECCIÓN DESCRIPTIVOS Y SOBRIOS: nombran el asunto, no lo dramatizan "
        "('Cambio del año por tipo de entidad', nunca 'La fractura estructural'). Evitá los "
        "giros de comentarista ('la cadena causal', 'el hallazgo central', 'lo que importa') "
        "y los anglicismos: es 'dentro del año' o 'intraanual', no 'intrayear'.\n\n"
        "LOS ACUMULADOS SE COPIAN, NO SE SUMAN. `concentracion_por_provincia` y "
        "`concentracion_por_sector` traen la cuota conjunta de los 2, 3 y 5 mayores YA "
        "computada sobre el crudo, con sus miembros nombrados. Usá ÉSAS. Sumar a mano dos "
        "pesos ya redondeados da un tercer número que ninguna fila respalda: en un informe "
        "real la suma de dos capítulos dio 42,2 cuando el dato era 42,3. Y nombrá SIEMPRE la "
        "población del acumulado —provincias o sectores—, que en este contexto viajan las "
        "dos.\n\n"
        "No recalcules ninguna cifra: todas vienen resueltas."
    ),
    "revision_anual_mercado": (
        ...
        "EL CONTRASTE VIENE RESUELTO, no lo recalcules. 'vs_su_tipo' y 'vs_el_sistema' traen "
        "la brecha, el sentido (mejor / peor / en línea) y la lectura ya redactada: copiala. Y "
        "'es_idiosincratico' es el VEREDICTO de esta sección, con su 'es_idiosincratico_por_que' "
        "— si es falso, el movimiento acompañó al estrato y presentarlo como mérito o demérito "
        "propio de la entidad sería falso; si es verdadero, desarrollalo primero. "
        "Un campo en null significa que faltó el dato del estrato: DECILO, no lo rellenes — "
        "pero decilo en PROSA. Nunca nombres los campos del contexto ni la palabra «null» en "
        "el texto: «no se pudo comparar contra su estrato en este corte» es la forma; «los "
        "campos `vs_su_tipo` y `es_idiosincratico` llegan en null» es vocabulario interno "
        "filtrándose a un documento que se vende, y ocurrió.\n\n"
        "Nombrá el estrato por su etiqueta ('tipo_de_entidad'), nunca por su clave técnica.\n\n"
        "No repitas la descripción del año —ya la da la otra sección—: acá va solo el "
        "contraste y lo que implica.\n\n"
        "Prosa SOBRIA, sin giros de comentarista: es un veredicto analítico, no una columna.\n\n"
        "No recalcules ninguna cifra: todas vienen resueltas."
    ),
    "sector_outlook": (
        ...
        "cada dimensión trae su 'provenance' COMPUTADA ('real', 'real en parte', 'rúbrica "
        "declarada' o 'no declarada') y, cuando corresponde, 'sobre_rubrica_declarada' con las "
        "variables que sí son juicio de casa. Apóyate con firmeza en lo real; sobre una variable "
        "de rúbrica no construyas conclusión fuerte y nómbrala como rúbrica cuando sea material. "
        "NO clasifiques una dimensión por tu cuenta ni des por rúbrica lo que el contexto declara "
        "real: la lista está en el contexto y cambia con cada conector. Si una dimensión viene "
        "'no declarada', no afirmes nada sobre su procedencia.\n\n"
        "QUÉ EXPLICA EL SCORE: cada dimensión trae 'que_hay_dentro' con sus variables y el valor "
        "YA en la unidad en que se cita. Ahí está la frase accionable: no digas solo que una "
        "dimensión lastra, nombra la variable y su valor —p. ej. la tasa a la que el sistema "
        "financiero le presta al sector, o el salario promedio cotizable—. Copia los valores tal "
        "como vienen (la unidad va en el nombre de la variable) y no conviertas razones a "
        "porcentajes ni al revés.\n\n"
        "PARA UBICAR AL SECTOR usá 'puesto_del_sector', que ya viene escrito entero con su "
        "población adentro ('13 de los 16 sectores con dato de costo del capital'). COPIALO CON "
        "SU POBLACIÓN y en CIFRAS: el denominador cambia de una variable a otra —no toda fuente "
        "alcanza a los 17 sectores— y arrastrar el de la variable de al lado, o escribirlo en "
        "palabras, convierte una cifra verificable en una que nadie puede contrastar. 'posicion_en_la_escala_de_valor_del_panel_0_100' es OTRA cosa y "
        "NO es un percentil: es dónde cae el VALOR entre el mínimo y el máximo del panel, así "
        "que NUNCA la llames percentil ni digas que un porcentaje de los sectores está por "
        "debajo. Si querés hablar de posición, usá el puesto.\n\n"
    ...
    "banking_sector_map_system": (
        ...
        "LAS CIFRAS SE COPIAN, y los ACUMULADOS también: `concentracion_por_sector` y "
        "`concentracion_por_provincia` traen la cuota conjunta de los 2, 3 y 5 mayores ya "
        "computada, con sus miembros nombrados — usá ésas en vez de sumar. Pesos, moras y "
        "tasas vienen computados. No sumes los pesos "
    ...
    "banking_sector_map": (
        ...
        "Contexto ('mapa_sectorial.sectores': una fila por sector, con la exposición de la "
        "entidad y su referencia del resto del sector):\n{context}\n\n"
        "Máximo 350 palabras. EMPIEZA POR EL VEREDICTO: de los sectores donde la entidad "
        "está peor que el resto del sistema, ¿cuál explica más de su morosidad agregada, y "
        "es un problema de originación suya o del sector? Desarrolla 2-3 sectores, no la "
        "lista: los de mayor exposición combinada con mayor brecha.\n"
        "LA GEOGRAFÍA, cuando el contexto trae 'mapa_sectorial.provincias'. Dice DÓNDE "
        "presta la entidad contra dónde presta el país. 'sobre_representacion_pp' viene "
        "computada: positiva significa que la entidad pesa MÁS en esa provincia que el "
        "mercado. Las dos cuotas tienen denominadores distintos —'peso_en_su_cartera_pct' "
        "es de su libro y 'peso_de_la_provincia_en_el_pais_pct' del crédito nacional—: al "
        "citar una, di de qué es. Menciona 2-3 provincias, no las treinta y tres, y solo si "
        "la concentración o la brecha de mora dicen algo. «SIN PROVINCIA» no es un lugar: es "
        "la porción sin rótulo en la fuente; si es material, nómbrala así y no como una "
        "región.\n"
        "REGLA DURA — NO SUMES NI PROMEDIES PORCENTAJES. `concentracion_por_sector` trae "
        "la cuota conjunta de los 2, 3 y 5 sectores mayores de ESTA entidad, ya computada "
        "sobre el crudo y con sus miembros nombrados: si vas a hablar de un grupo, es esa "
        "cifra. `mapa_sectorial.resumen` ya trae "
        "los agregados: cuántos sectores caen en cada tipo de atribución y qué peso de la "
        "cartera representan. Usá ESOS. Sumar el peso de dos sectores que elegiste vos "
        "produce una cifra que ninguna fila respalda —y además dice menos: el primer "
        "informe real sumó los dos sectores mayores y publicó «el 48,39%» cuando los "
        "sectores con deterioro propio eran cinco y pesaban el 58,7%, así que la selección "
        "subestimaba el hallazgo—. Si querés hablar de un subconjunto, nombrá el peso de "
        "cada sector por separado y no su suma.\n"
        "REGLA DURA — LAS RELACIONES YA ESTÁN COMPUTADAS Y SE COPIAN. "
        "'brecha_de_mora_pp' y 'spread_de_tasa_pp' vienen calculados en el contexto: "
        "cópialos con su signo. NO los recalcules restando dos porcentajes ni deduzcas la "
        "dirección de la comparación por tu cuenta. Un signo positivo en la brecha significa "
        "que la entidad tiene MÁS mora que el resto del sector; uno negativo, MENOS. "
        "'atribucion' ya dice cuál de las dos causas aplica: úsala, no la re-deduzcas.\n"
        "REGLA DURA — CONTRA QUÉ SE COMPARA. Toda referencia del contexto es el RESTO del "
        "sector EXCLUIDA esta entidad, y así debes nombrarla ('el resto del sistema en "
        "construcción'), nunca «el sector» a secas ni «el promedio del sistema». La única "
        "cifra que es sobre el sector ENTERO es 'cuota_del_sector_pct', que es su "
        "participación, no una comparación.\n"
        "EL SUJETO VIAJA CON EL NÚMERO. 'peso_en_su_cartera_pct' es sobre la cartera de la "
        "entidad; 'cuota_del_sector_pct' es sobre el sector; 'peso_del_sector_en_el_sistema_pct' "
        "es sobre el crédito del país. Son tres denominadores distintos: al citar cualquiera, "
        "di de qué es porcentaje.\n"
        "SI HAY TASA, LA LECTURA ES DOBLE. Compara 'brecha_de_mora_pp' con 'spread_de_tasa_pp' "
        "en el mismo sector: cobrar más por un riesgo peor es precio del riesgo; cobrar lo "
        "mismo o menos por un riesgo peor es riesgo no cobrado, y ése es el hallazgo. Si "
        "'tasa_promedio_ponderada_pct' viene nula, no menciones precio en ese sector.\n"
        "LA CAPACIDAD DE PAGO DEL DEUDOR, cuando el contexto trae 'capacidad_de_pago'. "
        "Úsala SOLO si el sector de consumo tiene exposición material en esta entidad, y "
        "SOLO para explicar su mora: el crédito de consumo se concentra en los hogares de "
        "menor ingreso, así que lo que puede pagar ese deudor no lo dice ningún estado "
        "financiero. Tres lecturas, y cada una responde algo distinto:\n"
        "  · 'inflacion_del_deudor' — la inflación acumulada de la canasta de CADA quintil "
        "de ingreso, NO la del índice general. Cita "
        "'brecha_quintil_1_menos_quintil_5_pp' tal cual viene —está computada— y di sobre "
        "qué ventana se mide ('desde'/'hasta').\n"
        "  · 'salario_minimo' — el piso de ingreso. Nómbralo con su categoría, nunca como "
        "«el salario mínimo» a secas: hay once y varias están congeladas. Si el bloque trae "
        "'congelada' en verdadero, DILO: esa referencia no describe el presente.\n"
        "  · 'costo_de_la_canasta' y 'cobertura_del_piso_de_ingreso' — lo que CUESTA en "
        "pesos la canasta de cada quintil, y qué porción de ella alcanza a cubrir el salario "
        "mínimo. La cobertura viene COMPUTADA: cópiala, no la calcules. Por debajo de 100 "
        "significa que un hogar de ese quintil, con un solo ingreso al piso legal, no llega "
        "a su propia canasta — dilo así, no como que ese hogar gane el mínimo.\n"
        "  · 'mercado_laboral' — el trimestre. Un ocupado informal no tiene ingreso "
        "verificable, y el crédito de consumo se origina contra ingreso declarado. Y hay "
        "DOS medidas de holgura: 'desocupacion_abierta_su1_pct' solo ve a quien busca y no "
        "encuentra; 'subutilizacion_amplia_su4_pct' suma al subocupado por horas y a la "
        "fuerza de trabajo potencial. Si citás una sola, citá la AMPLIA y nómbrala como tal "
        "—la angosta sola subestima la holgura a menos de la mitad—; "
        "'holgura_que_SU1_no_ve_pp' viene computada, cópiala.\n"
        "  · 'holgura_donde_presta' — la holgura laboral del TERRITORIO donde la entidad "
        "presta, no la del país. 'subutilizacion_ponderada_por_su_exposicion_pct' está "
        "ponderada por su cartera y 'mas_holgura_que_el_pais_pp' es la diferencia contra el "
        "país: las dos vienen computadas, copialas. Este cruce es la mitad que un banco no "
        "tiene —los datos son públicos, el libro de crédito abierto por provincia de las "
        "otras noventa y una entidades no— así que vale nombrarlo. Si "
        "'cartera_sin_dominio_asignable_pct' es material, dilo: es cartera cuya provincia la "
        "fuente no rotula, no un territorio.\n"
        "    LA ENCFT ES UNA ENCUESTA. Si el contexto trae 'precision_de_la_encuesta', esas "
        "cifras son ESTIMACIONES con error de muestreo, no censos. Cuando cites una "
        "diferencia entre dos de ellas, mirá "
        "'la_brecha_entre_SU1_y_SU4_es_significativa': si viene en falso, los intervalos se "
        "solapan y la diferencia NO se puede afirmar — decilo así en vez de darla por "
        "hecha. No inventes intervalos que el contexto no traiga, y no conviertas un "
        "intervalo en una precisión falsa citando más decimales de los que trae.\n"
        "NO las presentes como causa probada del deterioro: son el contexto que lo hace "
        "plausible, y la atribución de la mora ya la da 'atribucion'. Menciona SOLO las "
        "claves que el contexto trae: si falta una, no existe para este informe.\n"
        "NO NARRES COMO SEÑAL lo que trae 'atribucion' igual a 'exposicion_no_material' "
        "(la celda es demasiado chica y un solo crédito la mueve) ni a "
        "'sin_resto_con_que_comparar' (la entidad es la ÚNICA que presta a ese sector, así "
        "que no hay referencia). Este último SÍ vale nombrarlo como hallazgo: estar solo en "
        "un sector es una posición, no un dato faltante.\n"
        "Cierra con el sector que vigilarías al próximo corte y por qué. Esta lectura es "
        "CONTEXTO que explica los indicadores del cuadro de mando: no la presentes como un "
        "componente del score ni inventes que modifica la calificación."
    ),
    ...
    "energy_outlook": (
        ...
        "Y —cuando el contexto los traiga— el FINANCIAMIENTO del sector y su COSTO "
        "LABORAL: usá también credito_del_sistema_al_sector_energia y "
        "costo_laboral_del_sector_energia. Es la capa que ninguna otra fuente de este sector "
        "trae: cuánto crédito recibe, a qué tasa, con qué mora y con qué cobertura de "
        "provisiones. Un parque que se expande con crédito caro o mora en ascenso no financia la misma inversión que uno con financiamiento holgado. Cada una de esas dos capas trae SU propia fecha "
        "(corte_de_esta_capa, anio_de_esta_capa) y NO es la del índice: si las citás, "
        "decí a qué fecha corresponden. Si esas claves NO están en el contexto, no las "
        "menciones ni digas que faltan. "
        "Y si el contexto trae inversion_extranjera_en_el_sector_energia, ésa es la pregunta que la dimensión de capacidad NO puede contestar: el parque se expande o no, pero la IED dice si el capital efectivamente ENTRÓ a financiar esa expansión, o si el ritmo se sostiene con endeudamiento local. Un parque que crece con IED en retroceso está financiando su expansión de otra manera, y eso condiciona cuánto más puede crecer. Si trae actividad_del_sector_energia_en_las_cuentas_nacionales, leela como el TAMAÑO del insumo: la energía pesa poco en el valor agregado y condiciona a todo el resto, así que su crecimiento no se juzga contra el de un sector cualquiera. Y si trae ocupacion_de_la_rama_del_sector_energia, notá que es un sector de capital y no de empleo: los ocupados son pocos por diseño y no miden su importancia. Cada una de estas capas trae SU propia fecha (anio_de_esta_capa, periodo_de_esta_capa) y NO es la del índice: si las citás, decí a qué fecha corresponden. Y cuando una capa traiga un CAMBIO ABSOLUTO (…_usd_mm, …_vs_anio_anterior) al lado de una variación porcentual, citá el cambio absoluto: hay series que se miden NETAS y pueden ser negativas, y ahí el porcentaje viene vacío a propósito porque sobre una base negativa invertiría la dirección. Si alguna clave NO está en el contexto, no la menciones ni digas que falta. "
    ...
    "tourism_outlook": (
        ...
        "Y —cuando el contexto los traiga— el FINANCIAMIENTO del sector y su COSTO "
        "LABORAL: usá también credito_del_sistema_al_sector_turismo y "
        "costo_laboral_del_sector_turismo. Es la capa que ninguna otra fuente de este "
        "sector trae: cuánto crédito recibe, a qué tasa, con qué mora y con qué "
        "cobertura de provisiones. Un ciclo de llegadas al alza con crédito caro o mora en ascenso no sostiene la misma inversión hotelera que uno con financiamiento holgado. Cada una de esas dos capas trae SU propia "
        "fecha (corte_de_esta_capa, anio_de_esta_capa) y NO es la del índice: si las "
        "citás, decí a qué fecha corresponden. Si esas claves NO están en el contexto, "
        "no las menciones ni digas que faltan. "
        "Y si el contexto trae capacidad_de_pago, úsala con ESTA lectura: la DEMANDA INTERNA. El índice mide llegadas de no residentes; la capacidad de pago del hogar dominicano dice qué tan expuesto está el sector a depender de esa demanda externa, porque un mercado interno con el piso de ingreso por debajo de la canasta no amortigua una caída de llegadas. No "
        "es una capa del índice: trae su propio año y NO es el del informe, así que si la "
        "citás decí a qué año corresponde. Si no está, no la menciones ni digas que falta. "
        "Y si el contexto trae inversion_extranjera_en_el_sector_turismo, ésa es la OFERTA que el índice reconoce no medir: las llegadas son demanda, y la IED en turismo es la planta hotelera que se está construyendo para recibirla. Llegadas al alza con IED en retroceso anticipan una restricción de capacidad; llegadas planas con IED fuerte anticipan sobreoferta. Si trae actividad_del_sector_turismo_en_las_cuentas_nacionales, es cuánto del valor agregado nacional depende de este sector — la exposición del país, no la del sector. Y si trae ocupacion_de_la_rama_del_sector_turismo, es el empleo que sostiene: la traducción de las llegadas a puestos de trabajo. Cada una de estas capas trae SU propia fecha (anio_de_esta_capa, periodo_de_esta_capa) y NO es la del índice: si las citás, decí a qué fecha corresponden. Y cuando una capa traiga un CAMBIO ABSOLUTO (…_usd_mm, …_vs_anio_anterior) al lado de una variación porcentual, citá el cambio absoluto: hay series que se miden NETAS y pueden ser negativas, y ahí el porcentaje viene vacío a propósito porque sobre una base negativa invertiría la dirección. Si alguna clave NO está en el contexto, no la menciones ni digas que falta. "
    ...
    "telecom_outlook": (
        ...
        "Y si el contexto trae capacidad_de_pago, úsala con ESTA lectura: la ASEQUIBILIDAD "
        "del servicio. El salario mínimo contra el costo de la canasta dice cuánto queda "
        "disponible en un hogar después de comer, y ése es el techo real de la penetración "
        "de banda ancha: el margen que falta para llegar a los hogares que todavía no "
        "tienen internet no es de cobertura sino de bolsillo. No es una capa del índice: "
        "trae su propio año y NO es el del informe, así que si la citás decí a qué año "
        "corresponde. Si no está, no la menciones ni digas que falta. "
        "Y si el contexto trae inversion_extranjera_en_el_sector_comunicaciones, ésa es la señal de CAPEX que un índice de penetración no puede ver: la penetración es el resultado de una red que ya se construyó, y la IED en telecomunicaciones dice si se está construyendo la siguiente. Penetración estancada con IED en alza es inversión que todavía no se refleja; estancada con IED en caída es un techo. Si trae actividad_del_sector_comunicaciones_en_las_cuentas_nacionales, notá que el sector pesa poco en el valor agregado y eso NO contradice su importancia: es infraestructura habilitante, y su peso mide su facturación, no su efecto. Y si trae ocupacion_de_la_rama_del_sector_comunicaciones, OJO: la ENCFT agrupa transporte y comunicaciones en una sola rama, así que esa cifra viene marcada como AGREGADO — si la citás, decí de qué rama es y no la atribuyas solo a telecom. Y si trae costo_laboral_del_sector_comunicaciones, es el salario promedio cotizable del sector. Cada una de estas capas trae SU propia fecha (anio_de_esta_capa, periodo_de_esta_capa) y NO es la del índice: si las citás, decí a qué fecha corresponden. Y cuando una capa traiga un CAMBIO ABSOLUTO (…_usd_mm, …_vs_anio_anterior) al lado de una variación porcentual, citá el cambio absoluto: hay series que se miden NETAS y pueden ser negativas, y ahí el porcentaje viene vacío a propósito porque sobre una base negativa invertiría la dirección. Si alguna clave NO está en el contexto, no la menciones ni digas que falta. "
    ...
    "free_zones_outlook": (
        ...
        "Y —cuando el contexto los traiga— el FINANCIAMIENTO del sector y su COSTO "
        "LABORAL: usá también credito_del_sistema_al_sector_zonas_francas y "
        "costo_laboral_del_sector_zonas_francas. Es la capa que ninguna otra fuente de este sector "
        "trae: cuánto crédito recibe, a qué tasa, con qué mora y con qué cobertura de "
        "provisiones. OJO: la SIB no separa zonas francas de la manufactura local, así que esa cifra puede venir marcada como AGREGADO — si viene así, decí de qué agregado es y no la atribuyas solo a zonas francas. Cada una de esas dos capas trae SU propia fecha "
        "(corte_de_esta_capa, anio_de_esta_capa) y NO es la del índice: si las citás, "
        "decí a qué fecha corresponden. Si esas claves NO están en el contexto, no las "
        "menciones ni digas que faltan. "
        "Y si el contexto trae capacidad_de_pago, úsala con ESTA lectura: el COSTO LABORAL y la disponibilidad de mano de obra. El salario mínimo y la subutilización amplia (SU4) dicen contra qué compite el régimen por trabajadores; la informalidad dice cuánta fuerza laboral queda fuera del empleo formal que las zonas francas ofrecen. No "
        "es una capa del índice: trae su propio año y NO es el del informe, así que si la "
        "citás decí a qué año corresponde. Si no está, no la menciones ni digas que falta. "
        "Y si el contexto trae inversion_extranjera_en_el_sector_zonas_francas, ésa es la confirmación externa del atractivo que el índice infiere: exportaciones y empleo dicen cómo le va al régimen instalado, y la IED dice si SIGUE llegando capital nuevo a instalarse. Un régimen que exporta más con IED en caída está creciendo por las empresas que ya están, no por las que vienen. Si trae actividad_del_sector_zonas_francas_en_las_cuentas_nacionales, es el peso del régimen en el valor agregado nacional — la escala real de lo que se está discutiendo. Y si trae ocupacion_de_la_rama_del_sector_zonas_francas, OJO: la ENCFT no separa zonas francas de la manufactura local ni de la minería, así que esa cifra viene marcada como AGREGADO — si la citás, decí de qué rama es y no la atribuyas solo a zonas francas. Cada una de estas capas trae SU propia fecha (anio_de_esta_capa, periodo_de_esta_capa) y NO es la del índice: si las citás, decí a qué fecha corresponden. Y cuando una capa traiga un CAMBIO ABSOLUTO (…_usd_mm, …_vs_anio_anterior) al lado de una variación porcentual, citá el cambio absoluto: hay series que se miden NETAS y pueden ser negativas, y ahí el porcentaje viene vacío a propósito porque sobre una base negativa invertiría la dirección. Si alguna clave NO está en el contexto, no la menciones ni digas que falta. "
    ...
    "construction_outlook": (
        ...
        "diversificación tipológica y amplitud geográfica (HHI), y —cuando el contexto los "
        "traiga— el FINANCIAMIENTO del sector y su COSTO LABORAL. Usa EXCLUSIVAMENTE las "
        "cifras del contexto (icc_score, coverage, construction_gdp_growth_3y_pct, los "
        "niveles del año —permits, sqm_licensed, investment_licensed_mm_dop, "
        "top_typology/top_province—, el score/metric por dimensión y, si están, "
        "credito_del_sistema_al_sector_construccion y costo_laboral_del_sector_construccion). "
        "EL FINANCIAMIENTO es la capa que ninguna otra fuente de este sector trae: cuánto "
        "crédito recibe la construcción, a qué tasa, con qué mora y con qué cobertura de "
        "provisiones. Un ciclo de permisos a la baja con crédito caro o mora en ascenso no "
        "es la misma lectura que uno con financiamiento holgado, y esa distinción es la "
        "que decide una exposición. Si esas claves NO están en el contexto, no las "
        "menciones ni digas que faltan: simplemente no forman parte de esta lectura. Cada "
        "una de esas dos capas trae SU propia fecha (corte_de_esta_capa, anio_de_esta_capa) "
        "y NO es la del índice: si las citás, decí a qué fecha corresponden. Distingue actividad LÍDER (permisos, lo que viene) de "
        "PRODUCCIÓN realizada (PIB, lo ya hecho): si divergen, esa divergencia es la lectura. "
        "Y si el contexto trae capacidad_de_pago, úsala con ESTA lectura: la DEMANDA de vivienda. El salario mínimo contra el costo de la canasta dice qué puede pagar un hogar, y la inflación por quintil dice a quién le apretó más. Un pipeline de permisos que cae mientras el piso de ingreso no cubre la canasta es un problema de demanda solvente, no de oferta. Y si trae holgura_donde_construye, esa es la subutilización laboral de los territorios DONDE se licencia, ponderada por m²: dice si hay mano de obra donde se está construyendo. No "
        "es una capa del índice: trae su propio año y NO es el del informe, así que si la "
        "citás decí a qué año corresponde. Si no está, no la menciones ni digas que falta. "
        "Y si el contexto trae ocupacion_de_la_rama_del_sector_construccion, ésa es la OTRA mitad del ciclo que los permisos no ven: el flujo de licencias dice qué se va a construir y los ocupados de la rama dicen con cuánta gente se está construyendo HOY. Un pipeline que cae con el empleo del sector todavía en alza es un ajuste que aún no llegó al mercado laboral; uno que cae con los ocupados ya cediendo es un ajuste en curso, y son dos lecturas distintas para quien decide una exposición. Cada una de estas capas trae SU propia fecha (anio_de_esta_capa, periodo_de_esta_capa) y NO es la del índice: si las citás, decí a qué fecha corresponden. Y cuando una capa traiga un CAMBIO ABSOLUTO (…_usd_mm, …_vs_anio_anterior) al lado de una variación porcentual, citá el cambio absoluto: hay series que se miden NETAS y pueden ser negativas, y ahí el porcentaje viene vacío a propósito porque sobre una base negativa invertiría la dirección. Si alguna clave NO está en el contexto, no la menciones ni digas que falta. "
    ...
    "social_outlook": (
        ...
        "\n\nY si el contexto trae holgura_laboral_de_la_region, úsala: es la SUBUTILIZACIÓN "
        "AMPLIA (SU4) del territorio, que incluye al subocupado por horas y a la fuerza de "
        "trabajo potencial — gente que quiere trabajar y que la desocupación abierta NO cuenta. "
        "El IDM trae pobreza, informalidad e ingreso pero NO esto, y es la diferencia entre "
        "«hay poco empleo» y «hay gente disponible que el mercado no absorbe». La brecha contra "
        "el país viene YA CALCULADA (mas_subutilizacion_que_el_pais_pp): copiala, no la "
        "recalcules. OJO: la ENCFT publica por DOMINIO (cuatro) y no por región de desarrollo "
        "(diez), así que ese valor es el del dominio que contiene a la región — decilo así y "
        "no lo atribuyas a la región como medición propia. Trae su propio año, que no es "
        "necesariamente el del índice. Si no está, no la menciones ni digas que falta."
        "CIFRAS DERIVADAS: 'cifras_derivadas' YA trae el aporte y el gap al techo por dimensión, "
        "el líder vs la suma del resto, la dimensión de mayor gap y el orden por peso. Copia esos "
        "valores; NO recalcules aportes ni declares un superlativo (mayor/menor/el más…) que no "
