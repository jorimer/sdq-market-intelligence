...
def test_create_and_resolve_current_price(db):
    out = create_tariff(db, sku="deep_dive:banking", amount="299.00")
    assert out["sku"] == "deep_dive:banking"
    assert out["amount"] == "299.00"
    assert out["is_current"] is True
    row = price_for(db, "deep_dive:banking")
    assert row is not None and row.amount == Decimal("299.00")


def test_amount_keeps_cents(db):
    create_tariff(db, sku="deep_dive:banking", amount=199.9)
    assert price_for(db, "deep_dive:banking").amount == Decimal("199.90")
...
def test_replacement_picks_latest_effective_from(db):
    ...
    create_tariff(db, sku="deep_dive:banking", amount="349",
                  effective_from=now + timedelta(days=5))   # programado
    # Hoy rige el viejo; tras la fecha programada, rige el nuevo.
    assert price_for(db, "deep_dive:banking", at=now).amount == Decimal("299")
    assert price_for(db, "deep_dive:banking", at=now + timedelta(days=6)).amount == Decimal("349")


def test_withdraw_excludes_from_resolution(db):
    out = create_tariff(db, sku="deep_dive:banking", amount="299")
...
def test_list_filters_by_sku(db):
    create_tariff(db, sku="deep_dive:banking", amount="299")
    create_tariff(db, sku="deep_dive:macro", amount="199")
...
def test_aware_datetime_normalized_to_naive(db):
    ...
    create_tariff(db, sku="deep_dive:banking", amount="10", effective_from=aware)
    stored = db.query(Tariff).filter_by(sku="deep_dive:banking").one()
    assert stored.effective_from.tzinfo is None
    assert stored.effective_from == _naive(aware)


# ── v2: intervalos (mensual/anual en suscripciones; once en compra puntual) ──
def test_interval_prices_resolve_independently(db):
    create_tariff(db, sku="all_access", interval="monthly", amount="1200")
    create_tariff(db, sku="all_access", interval="annual", amount="12000")
    assert price_for(db, "all_access", "monthly").amount == Decimal("1200")
    assert price_for(db, "all_access", "annual").amount == Decimal("12000")
    assert price_for(db, "all_access", "once") is None  # once no aplica a una suscripción


def test_interval_invalid_for_sku_rejected(db):
    with pytest.raises(TariffError):  # una suscripción no acepta 'once'
        create_tariff(db, sku="all_access", interval="once", amount="10")
    with pytest.raises(TariffError):  # una compra puntual no acepta 'monthly'
        create_tariff(db, sku="deep_dive:banking", interval="monthly", amount="10")
