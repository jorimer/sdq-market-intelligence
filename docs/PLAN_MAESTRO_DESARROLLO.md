...
- **G3 — Deal Scoring → FASES 1 y 2 EN PROD (2026-06-21, PRs #225 #231).** Módulo `deal_scoring` con `HistoricalDeal` (registro = dataset). **Análisis profundo de 2 backups** (OneDrive + LaCie, ~5,300 docs + email, workflows multi-agente) → set validado de ~29-36 labels balanceados (A: deals SDQ; B: real-estate CBS), persistido gitignoreado. **Fase 1:** rúbrica declarada (índice 0-100, NO modelo entrenado) anclada a los 7 ejes (`market_validation`←IAI, `regulatory_readiness`←IRMP, clima←IRC); rúbrica pura en el módulo + composición en `app/`; página + narrativa Claude. **Fase 2:** registro (import CSV admin + cosecha) + **curva de aprendizaje** (CV-AUC + IC bootstrap) cuya **graduación rúbrica→XGBoost es por el IC del AUC > umbral, NO por un "200" arbitrario** (sin sustento; el proto muestra AUC inestable a N=200; regla EPV). Seed versionado ANONIMIZADO (`DEAL-NNN`); datos reales solo en prod (DB privada) + inventario gitignoreado. **Fase 2.5 — gate de graduación libre de fuga (#233, 2026-06-21):** backfillear los 36 labels retrospectivos inflaba el AUC 0.27→0.96 y graduaba el modelo falsamente (**label leakage**, no skill). Fix: columna `retrospective` (migración marca pre-existentes como retrospectivas; `import`→retrospectivo, `save_deal`→ex-ante); **la curva gradúa SOLO sobre labels ex-ante (libres de fuga)**; los retrospectivos van a un `retrospective_diagnostic` marcado "no gradúa". **134 deals + backfill de las 4 señales (juicio AI-native, aprobado por el dueño) cargados a prod.** Verificado: `n_ex_ante=0, status=rubrica`; diagnóstico retrospectivo 0.962 aislado del gate. **Pendiente:** resolver los 98 "open" → cosecha going-forward (scorear deals nuevos antes del outcome = únicos ex-ante que gradúan) → XGBoost cuando la curva ex-ante gane el IC. Cierra parcialmente G2 (captura de outcomes). Privados en `docs/Modelos Propietarios/`.
...
- **Deal Scoring** (`Modelos Propietarios/deal_scoring.py`) — integrar como módulo formal (api/models/tests). IP sin capitalizar.
  - **Datos de entrenamiento decididos (2026-06-20):** no hay fuente pública que entrene el modelo (4/10 features son scores propietarios de analista SDQ; falta de negativos por sesgo de supervivencia; ToS de Crunchbase prohíbe entrenar). Entrenar con los **200+ deals propios** (spec §3.4); Crunchbase solo para priors agregados (carve-out ToS), World Bank PPI para estructura. Ver `Modelos Propietarios/DEAL_SCORING_Datos_Entrenamiento_Decision.md` + prototipo de validación de pipeline en `Modelos Propietarios/deal_scorer_proto/`.
...
- **RBAC + administración de usuarios** — **EN PROD (#235 backend, #236 frontend, 2026-06-21).** Rol jerárquico super_admin/admin/analyst/viewer + tier free/pro/enterprise (monetización, declarado sin cobrar) + CRUD gateado en `/api/v1/admin/users` con barandas anti-escalada/anti-lockout + página Administración→Usuarios. Bootstrap del super_admin real por env (`SUPERADMIN_EMAIL`/`SUPERADMIN_PASSWORD`). Ver [[rbac-user-admin]].
- **i18n multilingüe ES/EN/FR** — **por fases.** Fase 1 (toggle + marco, #238) y Fase 2 (narrativas IA por idioma, #240) **EN PROD**; **Fase 3 en curso** (barrido por eje): ESG #242 · Regulatorio #244 · Sectorial #245 · Social #246 · Comercio #248 · Macro #249 · Financiero 3/4 (#250/#251/#252) en prod. Pendiente: Financiero sub-PR 4 (Reports/Model/Validation + drawers) · Plataforma · sección Datos. Ver [[i18n-multilang]].
- **Seguridad pre-go-live** — **PARCIAL.** Falta: (1) el dueño setea `SUPERADMIN_EMAIL`/`SUPERADMIN_PASSWORD` en Railway → crea el super_admin real; (2) desactivar `claude@sdqconsulting.com.do` (`scripts/seed_e2e_user.py --deactivate`); (3) revisar exposición del repo público (secretos/datos). **Dejado para el final por decisión del dueño (2026-06-20).**
