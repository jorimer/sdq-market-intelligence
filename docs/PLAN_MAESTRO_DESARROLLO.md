...
- **G3 — Deal Scoring huérfano → scaffold iniciado (2026-06-20).** Creado `modules/deal_scoring/` con el modelo `HistoricalDeal` (registro de deals = dataset de entrenamiento), registrado en Alembic `env.py`, con seed de 35 oportunidades del pipeline propio (5 con datos reales, 3 cerrados / 2 negativos / 30 abiertos) y tests (5/5). **El seed se versiona ANONIMIZADO** (códigos `DEAL-NNN`, solo features estructurales; nombres/carpetas/notas reales viven en el inventario privado gitignoreado — el repo es público). Migración Alembic `f8b3d1a6c904` creada y verificada (`upgrade head` + rollback `-1` limpios en SQLite). Script de backfill guiado `scripts/backfill_deal_scores.py` (export-csv/import-csv/interactive) probado round-trip. **Pendiente:** correr la migración en prod, backfillear las 4 señales de analista (juicio SDQ) y poblar hacia las 200, api/router/evaluator XGBoost (el `deal_scoring.py` aún no es módulo formal con inferencia). Cierra parcialmente la captura de outcomes para deals (intersección con G2). Estrategia de datos + inventario privados en `docs/Modelos Propietarios/` (gitignoreado).
