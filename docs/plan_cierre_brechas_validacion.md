...
**Origen:** [`docs/certificacion_validacion_catalogo_mip.md`](certificacion_validacion_catalogo_mip.md) (2026-08-16, commit `2721779`)
...
fija. Ninguno se recalcula cuando cambia el dato o el score que lo produjo. Estado real en producción:
...
### Acciones
1. Recalcular por consola las siete operaciones de validación:
   `backtest`, `irmp-backtest`, `trade-backtest`, `esg-backtest`, `idm-convergent-validity`,
   `insurance-backtest`, `pension-backtest` (vía `POST /api/v1/operations/{name}/run`).
...
4. Anotar los deltas contra la certificación del 16-ago (ya sabemos que banca y seguros se movieron;
   hay que ver si irmp/trade/esg/sector también).

### Status de avance — criterios de cierre
- [ ] Los 7 reportes tienen `generated_at` dentro de las 24 h de la corrida.
- [ ] `evidence/validacion_baseline_*.json` comiteado, con las 7 entradas y sus deltas.
- [ ] Cada eje tiene veredicto explícito: `concluyente` · `no concluyente` · `nulo` · `sin motor`.
- [ ] **Cero** ejes con motor y sin cifra vigente.

> **Ya ejecutado en la certificación:** `backtest` (Gini 0.1615) y `insurance-backtest` (0.0927 /
> 0.1563, ninguna concluyente). Faltan cinco.
...
1. **Invalidación por evento, no por reloj.** Cablear cada recálculo de validación al evento que lo
   invalida, sobre `shared.events.event_bus` (el mismo patrón con que `tpm-model-train` se dispara
   tras `bcrd-comunicados-sync`):
...
3. **Habilitar los schedules apagados** (`insurance-backtest`, `pension-backtest`) y **corregir las
   cadencias absurdas**: ESG pasa de anual a la cadencia de su fuente; el reloj queda como red de
   seguridad, no como mecanismo primario.
4. **Test estructural** (`ast`) que recorre `modules/*/validation/` y exige, para cada motor: (a) un
   ...
   Es la cura que la doctrina exige cuando un defecto reincide entre motores. Al escribir el glob,
   declarar explícitamente qué queda afuera (`tpm_modeling` y `sib_historical_backtest` no viven bajo
   `validation/`).
...
- [ ] Los 7 reportes exponen `input_fingerprint`; con fingerprint desfasado el endpoint responde
      `stale: true`.
- [ ] `insurance-backtest` y `pension-backtest` habilitadas; ESG con cadencia de fuente.
...
3. **Corregir el texto del caveat** ([`validation/report.py:62`](../modules/banking_score/validation/report.py:62)):
   debe nombrar *cuál* banda viola la monotonía y con qué N, computado del propio resultado — no una
   frase fija que afirma "tiers intermedios". Es la doctrina de que las relaciones se **computan**,
   no se narran.
...
### Acciones
1. **Pensiones (C1).** Tiene resultado **concluyente** —señal `return`: Gini 0.1594, IC [0.103, 0.219],
   n=1.590, 665 eventos— del 2026-07-03, **invisible**: no hay ruta de validación en el `openapi.json`
   de producción. Exponer `GET /api/v1/pension-intel/validation` con el mismo contrato que seguros, y
   recalcular. Es la brecha más barata del catálogo: el motor ya está escrito y testeado.
