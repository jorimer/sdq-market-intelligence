...
def test_hay_UNA_transaccion_verificable_y_es_la_del_Progreso():
    verificables = [t for t in tx.PANEL if t.verificable]
    assert len(verificables) == 1
    t = verificables[0]
    assert t.adquirida == "Banco Dominicano del Progreso"
    assert t.pb == pytest.approx(2.531, abs=0.001)
