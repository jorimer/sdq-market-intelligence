...
from modules.banking_score.reports.narrative import (
    NarrativaTruncada,
    _exigir_que_no_falte_ningun_pais,
...
def test_estan_los_tres_paises():
    textos = ["Chile — corte julio 2026…", "Panamá — corte julio 2026…",
              "El Salvador — corte junio 2026…"]
    _exigir_que_no_falte_ningun_pais(_BLOQUES, textos)
...
def test_un_texto_que_habla_de_OTRO_pais_tampoco_cuenta():
    """No alcanza con que haya texto: tiene que nombrar a SU país."""
    textos = ["Chile…", "Panamá…", "Un párrafo que no nombra a nadie"]
    with pytest.raises(NarrativaTruncada, match="El Salvador"):
        _exigir_que_no_falte_ningun_pais(_BLOQUES, textos)
