...
_SAMPLE_NARRATIVES = {
    "construction_pulse": (
        ...
        "Construcción de 26.5/100 —banda Crítico—**, una coyuntura claramente enfriada "
        "tras el auge post-pandemia. Las dos dimensiones cíclicas mandan: la **producción "
        "efectiva** (PIB real de construcción) promedia apenas **+0.39% anual** en los "
        "últimos tres años y cerró 2025 en **−1.8%**, y el **pipeline de permisos** se "
        "contrae con fuerza —los metros² licenciados caen a un **−7.5% anual** (CAGR 3 "
        "años), de 5.7M a 4.5M de m²—. La cara más sana es estructural: la actividad sigue "
        "**razonablemente diversificada** por tipología (apartamentos lideran con 59.6% de "
        "los permisos) y **bien distribuida geográficamente** (Santo Domingo concentra solo "
        "22.8%). La lectura de mercado: el sector más grande de la economía (13.5% del VAB) "
        "atraviesa una corrección —menos obra nueva en cartera— y la señal líder de los "
    ...
    "construction_assessment": (
        ...
        "deprimida por sus dos motores cíclicos y sostenida apenas por la diversificación. "
        "La **producción** real del sector se estancó (promedio 3 años +0.4%, último año "
        "−1.8%), y el **pipeline** —el mejor adelanto disponible— es negativo: con 951 "
        "licencias y 4.5M de m² en 2025, los permisos vienen contrayéndose ~7.5% anual. La "
        "**inversión licenciada** ronda los RD$271 mil millones (nominal). El contrapeso es "
        "que la base de actividad no depende de un solo segmento ni región: la "
        "diversificación tipológica (apartamentos 59.6%) y la amplitud geográfica (Santo "
        "Domingo 22.8%) puntúan alto. Para un inversionista, el cuadro es de **sector en "
        "corrección**: oportunidad selectiva donde la demanda estructural persiste, pero con "
        "el ciclo de permisos aún a la baja."
    ),
    "recommendation": (
        ...
        "la producción (PIB real) como el pipeline líder (permisos) apuntan a la baja, así "
        "que el timing de nueva exposición debe ser cauto y selectivo. La palanca de mayor "
        "valor no está en sumar capacidad horizontal sino en **leer el adelanto de los "
        "permisos por tipología y provincia**: la diversificación sana indica que hay "
        "segmentos y plazas donde la demanda estructural persiste (vivienda, plazas fuera "
        "del Gran Santo Domingo). Recomendación: condicionar las tesis a una reactivación "
        "del pipeline de permisos —el mejor indicador adelantado— antes de asumir un giro "
        "del ciclo, y priorizar nichos diversificados sobre apuestas concentradas."
    ),
}
